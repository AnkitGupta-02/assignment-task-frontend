...
const EditServiceForm = ({ service, onSubmit, onCancel }) => {
  ...
  // Load service data when component mounts or service changes
  useEffect(() => {
    if (service) {
      setFormData(service);
    }
  ...
  const removeSubService = (index) => {
    const updatedSubServices = formData.sub_services.filter(
      (_, i) => i !== index
    );
    setFormData({
      ...formData,
  ...
  // If no service data is provided, show a message
  if (!service) {
    return (
      <div className="p-6 bg-white rounded-lg">
        <p className="text-red-500">No service data provided for editing.</p>
        <button
          onClick={onCancel}
